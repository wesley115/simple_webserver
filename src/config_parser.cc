  ...
  while (input->good()) {
    ...
    switch (state) {
      ...
      case TOKEN_STATE_SINGLE_QUOTE:
        *value += c;
        if (c == '\'') {
          // end of a quoted token should be followed by whitespace
        ...
        // Allows for backslash-escaping within strings.
        if (c == '\\') {
  ...
  while (true) {
    ...
    if (token_type == TOKEN_TYPE_START) {
      ...
      if (last_token_type != TOKEN_TYPE_STATEMENT_END && 
          last_token_type != TOKEN_TYPE_START_BLOCK &&
          last_token_type != TOKEN_TYPE_END_BLOCK) {
        // fixes empty braces and double end braces bug
        // Error.
        break;
      }
      config_stack.pop();
      if (config_stack.empty()) {
        // Error: fixes unclosed braces bug for }
        break;
      }
      ...
      if (last_token_type != TOKEN_TYPE_STATEMENT_END &&
        ...
        // fixes empty config file bug and allows empty config
        // Error.
        break;
      } else if (config_stack.size() >= 2) {
        // Error: fixes unclosed braces bug for {
        break;
      }
      return true;
